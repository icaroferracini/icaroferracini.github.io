...
function criarMarcadores() {
  ...
  dadosMuseus.forEach(museu => {
    ...
    const statusBadge = status === 'fechado'
      ? '<span class="badge badge-vermelho">Não funcionando</span>'
...
// Adicionamos um parâmetro opcional 'nomeParaAbrirPopup'
function aplicarFiltros(nomeParaAbrirPopup = null) {
  ...
  setTimeout(() => {
    ...
    let marcadorEncontradoParaPopup = null; // Variável para armazenar o marcador a ser aberto
    ...
    function processChunk() {
      ...
      for (let i = processedCount; i < endIndex; i++) {
        ...
        // A correspondência de nome será tratada com prioridade
        const correspondeNomeExato = nomeSelecionado !== '' && nome.includes(nomeSelecionado);
        ...
        // --- NOVA LÓGICA: Priorizar o filtro por nome ---
        let deveExibir = false;
        if (nomeSelecionado !== '') { // Se há um nome digitado na busca
          deveExibir = correspondeNomeExato; // Apenas exibe se o nome corresponde
        } else { // Se não há nome digitado, aplica todos os outros filtros normalmente
          deveExibir = correspondeStatus && correspondeUF && correspondeAcess && correspondeTematica;
        }
        ...
        if (deveExibir) {
          ...
          // Se 'nomeParaAbrirPopup' foi fornecido e este marcador corresponde, armazene-o
          if (nomeParaAbrirPopup && nome === nomeParaAbrirPopup.toLowerCase()) {
            marcadorEncontradoParaPopup = marker;
          }
        }
      }
      ...
      if (processedCount < totalMarkers) {
        ...
        if (visiveisBounds.length > 0) {
          ...
          // Se um marcador foi encontrado para abrir o popup, abra-o
          if (marcadorEncontradoParaPopup) {
            marcadorEncontradoParaPopup.openPopup();
          }
...
Promise.all([
  ...
  inputNome.addEventListener('input', function () {
    ...
    if (val.length < 3) {
      // Não chama aplicarFiltros aqui para evitar filtro instantâneo
      return;
    }
    ...
    resultados.forEach(res => {
      ...
      div.addEventListener('click', function () {
        ...
        autocompleteList.innerHTML = '';
        aplicarFiltros(title); // Passa o nome para aplicarFiltros
      });
  ...
  inputNome.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') {
      e.preventDefault();
      autocompleteList.innerHTML = '';
      aplicarFiltros(inputNome.value); // Passa o nome para aplicarFiltros
    }
  });
...
limparNome.addEventListener('click', (e) => {
  ...
  aplicarFiltros(); // Chama aplicarFiltros ao limpar o campo, sem nome para popup
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const mapSection = document.getElementById('map-section'); // Obtenha a referência à seção do mapa
  ...
  function showSection(targetId) {
    ...
    if (targetSection) {
      ...
      // Se a seção do mapa for ativada, o Leaflet precisa ser invalidado para redesenhar
      if (targetId === 'map-section') {
        // Pequeno atraso para garantir que a div #map esteja visível antes de invalidar
        setTimeout(() => {
          map.invalidateSize();
          aplicarFiltros(); // Reaplica os filtros para garantir que o mapa esteja correto
        }, 100);
      }
    }
  }

  // Define a seção inicial como Mapa
  ...
  // Opcional: Se quiser que o item 'Mapa' seja ativo no carregamento, mesmo se o script estiver em 'defer'
  const initialActiveMenuItem = document.querySelector('.menu-item.active');
  if (!initialActiveMenuItem) {
    document.querySelector('.menu-item[data-target="map-section"]').classList.add('active');
  }
});